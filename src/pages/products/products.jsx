...
import { changeBrandId, changeCategory, changePrice, filterFunc, filterProducts, getBrand, getCategories, getProducts } from '../../entities/reducerc/Products'
...
const Products = () => {
  ...
  const obj = useSelector((store) => store.products.obj)
                ...
                {brands.map((el) => (
                  <div key={el.id} className='flex gap-[10px]'>
                    <input type="radio"
                      ...
                      value={el.id}
                      onChange={(e) =>{
                        dispatch(changeBrandId(e.target.value))
                      }}
                    />
